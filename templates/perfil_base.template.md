# 🧠 CÁPSULA DE CONTEXTO: [Tu Nombre]

Este documento sirve como la fuente de verdad única para la generación de contenido de CV, evitando alucinaciones y asegurando que cada dato esté respaldado por la experiencia real del usuario.

> **📌 Estado de canales:** [Indica si tu LinkedIn ya está optimizado o no]

## 👤 PERFIL PROFESIONAL (ETHOS)

- **Pilar Principal:** [Tu identidad profesional core, ej: "Full Stack Developer", "Data Engineer", "Product Manager"]
- **Identidad Profesional:** [2-3 líneas describiendo tu propuesta de valor única]
- **Motivación & Ethos:** [Qué te mueve, tus valores, tu "por qué"]
- **Diferenciador:** [Qué te hace único vs. otros con skills similares]
- **Liderazgo (Soft Power):** [Experiencia liderando equipos o proyectos, si aplica]

---

## 🎙️ VOICE PROFILE (Referencia de Marca)

> **Documento completo:** Ver `brand_voice.template.md` para especificaciones detalladas.

**Resumen de voz en 5 líneas:**
1. **Tono:** [Ej: Profesional pero accesible, técnico pero humano]
2. **Estructura:** [Ej: Storytelling con datos, bullets concisos]
3. **Balance:** [Ej: 50% técnico / 50% impacto de negocio]
4. **Personalidad:** [Ej: Curioso, orientado a resultados, colaborativo]
5. **Adaptación:** [Ej: Más dinámico para startups, más formal para corporativos]

---

## ⚖️ VALUE HIERARCHY (Jerarquía de Prioridades)

Cuando hay conflicto entre valores o decisiones de posicionamiento, priorizar en este orden:

1. **[Valor 1]** - [Por qué es el más importante]
2. **[Valor 2]** - [Descripción]
3. **[Valor 3]** - [Descripción]
4. **[Valor 4]** - [Descripción]
5. **[Valor 5]** - [Descripción]

---

## 🚫 NON-NEGOTIABLES (Restricciones Absolutas)

**NUNCA incluir en ningún output:**

### Sobre Experiencia:
- ❌ [Restricción específica sobre experiencia que no tienes]
- ❌ [Otra restricción]

### Sobre Métricas:
- ❌ Porcentajes o números no documentados en este perfil
- ❌ Métricas de impacto inventadas

### Sobre Posicionamiento:
- ❌ [Cosas que no debes afirmar]
- ❌ [Proyectos/skills en progreso que no deben presentarse como completados]

### Sobre Tono:
- ❌ Superlativos vacíos ("el mejor", "excepcional", "world-class")
- ❌ Lenguaje desesperado o servil

---

## 🔝 UNIVERSAL KILLER SUMMARY (Para LinkedIn / About)

> "[Tu elevator pitch de 3-4 líneas que funciona para la mayoría de contextos]"

---

## 🛠️ THE POWER STACK (Habilidades Duras)

| Categoría | Tecnologías (Dominio Actual) | En Aprendizaje |
|:----------|:-----------------------------|:---------------|
| **Lenguajes** | [Listar con nivel de experiencia] | [En progreso] |
| **Frontend** | [Frameworks, herramientas] | [En progreso] |
| **Backend & DB** | [Lenguajes, DBs, servicios] | [En progreso] |
| **Cloud/DevOps** | [Plataformas, herramientas] | [En progreso] |
| **Otros** | [Herramientas adicionales] | [En progreso] |

---

## 🚀 PROYECTO DESTACADO: [Nombre del Proyecto] | [Fecha Inicio] - [Presente/Fecha Fin]

_[Breve descripción del contexto del proyecto]_

- **Problema:** [Qué problema resolvió]
- **Logros Técnicos:**
  - **[Área 1]:** [Descripción con tecnologías específicas]
  - **[Área 2]:** [Descripción con tecnologías específicas]
  - **[Área 3]:** [Descripción con tecnologías específicas]
- **Resultado/Impacto:** [Métricas o resultados concretos si los hay]

---

## 💼 TRAYECTORIA PROFESIONAL

### [Título del Puesto] - [Empresa] | [Fecha Inicio] - [Fecha Fin]

_[Breve contexto del rol]_

- **Logro 1:** [Descripción con impacto]
- **Logro 2:** [Descripción con impacto]
- **Logro 3:** [Descripción con impacto]

### [Puesto Anterior] - [Empresa] | [Fecha Inicio] - [Fecha Fin]

_[Contexto]_

- **Logro 1:** [Descripción]

---

## 🎓 EDUCACIÓN

### [Institución] | [Título/Certificación]
- **Resumen:** [Qué aportó esta formación]

---

## 📜 CERTIFICACIONES & CURSOS

- **[Nombre de certificación]:** [Institución] ([Año])
- **[Curso en progreso]:** [Institución] (En curso)
